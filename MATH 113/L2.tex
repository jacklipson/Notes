...
\begin{example}[Octic Group $D_4$]
    Given a square: \begin{tikzpicture}
...
\end{proof}
\begin{remark}[Direct Sum of Groups]
...
\end{theorem}
\begin{proof}
    $\implies\colon$Consider the cyclic subgroup of $\Z_m\times\Z_n$ generated by (1,1). Clearly, the smallest number that is a multiple of both $m$ and $n$ will be $mn$ if and only if $\gcd(m,n) = 1$. It is at this number of summands that (1,1) yields the identity and implies $mn$ is the order of $\Z_m\times\Z_n$ and $\Z_{mn}$. Because \csgroup{(1,1)} is cyclic, they are isomorphic.
